...
async function loadProxyModule(): Promise<ProxyModule> {
  ...
  expect(loaded, "admin request ID proxy must exist").toBeDefined();
...
test("preserves a canonical request ID for the route and response", async () => {
  ...
  const response = proxy(
    new NextRequest("https://admin.example.invalid/healthz", {
...
test.each([
  ...
  const response = proxy(
    new NextRequest("https://admin.example.invalid/healthz", { headers }),
  );
