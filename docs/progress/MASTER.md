...
> 当前任务：`P1-04`（Lane A，Codex `/root`）；Lane B/C/D 无 executor
...
只领取位于 `ACTIVE` Phase、依赖已完成、状态为 `READY` 且对应 Lane 无 executor 的一个任务；当前 Lane A 由 `P1-04` 占用，Lane B/C/D 空闲，`P2-01` 为 `READY`。
...
这些 OPEN 项不阻塞当前 P1-04/P2-01，但执行者不得自行把 sandbox 假设写成生产结论。
